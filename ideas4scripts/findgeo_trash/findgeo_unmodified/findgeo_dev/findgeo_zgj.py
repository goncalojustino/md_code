#!/usr/bin/env python
##############################################################################
# as is, this scripts finds the correct metal ligands only
###############################################################################
# # -*- coding: utf-8 -*-
...
def main(code, input_file, output_dir, distance, metal):
	...
	if metals:
		...
		for key in ligands:
			print("-"*40)
			print "%s_%s%s_%s_%s --> %d ligands found." % \
			        (key.atype.strip(), key.resid, key.altConf, key.idx, key.chain, len(ligands[key]))
			...
			if len(ligands[key]) >= 2 and len(ligands[key]) <= 9: 
				...
				for line in pdb.query(key):
				 	#temp.writelines('%s\n' % line.output())
					print '%s\n' % line.output()
					#THE PREVIOUS LINE PRINTS THE METAL
				for element in ligands[key]:
					# temp.writelines('%s\n' % element.output())
					print '%s\n' % element.output()
					#THE PREVIOUS LINE PRINTS THE ATOMS NEARBY THE METAL
				temp.close()
...
def find_geo_input(pdb_name, output_dir, out):
	...
	if geom[0] != 'irr':
		...
		out.write('%s: %s - Irregular geometry\n' % (name, geom[0]))
...
if __name__ == '__main__':
	...
	if not options.pdb_file:
		#parser.error("A PDB file or PDB code is required.")
		parser.error("A PDB file is required.")
	...
	###############################################
	# MAYBE THIS COULD BE THOUGHT ABOUT A BIT MORE
	###############################################
