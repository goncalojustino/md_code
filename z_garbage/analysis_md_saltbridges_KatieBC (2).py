...
def plotter(distances, pos):
    ...
    ax.plot([0, 1000], [5, 5], 'k-', lw=2)
    ax.set_xlabel("time (ps)",fontsize=12)
    ax.set_ylabel(r"Pair distance ($\AA$)",fontsize=12)
...
if __name__ == '__main__':
    ...
    acid = universe.select_atoms(
        "(resname ASP and name OD*) or (resname GLU and name OE*) or (resname TYR and name OH) or "
        "(resname HIS and name ND*) or (resname HIS and name NE*) or (resname SER and name HG)")

    # selects all the basics
    basic = universe.select_atoms(
        "(resname ARG and name NH*) or (resname LYS* and name NZ*) or (resname HIS and name NE*) "
        "or (resname HIS and name ND*)")
